  ...
  return (
    <Card className="w-[600px] shadow-md">
      ...
      <CardContent className="space-y-4">
        ...
        {!user?.isOAuth && <div className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
          <p className="text-sm font-medium">2FA Authentication</p>
          <p className="truncate text-xs max-w-[180px] font-mono p-1 bg-slate-100 rounded-md">{user?.is2FAEnabled ? "Enabled" : "Disabled"}</p>
        </div>}
      </CardContent>
    </Card>
  );
